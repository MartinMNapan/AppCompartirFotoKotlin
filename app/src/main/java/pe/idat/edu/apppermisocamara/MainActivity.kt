...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun tomarFoto() {
        if (validarPermisoAlmacenamiento()){
            try {
                intencionTomarFoto()
    ...
    private fun intencionTomarFoto() {
        ...
        if (tomarFotoIntent.resolveActivity(this.packageManager) != null){
            ...
            if(archivoFoto != null){
                val uriFoto = obtenerUri(archivoFoto)
    ...
    private fun compartirFoto() {
        if(rutaFotoActual != ""){
            ...
            Toast.makeText(applicationContext, "Debe tomar en foto para compartir",
        ...
        if(requestCode == 1999){
            if (grantResults.isNotEmpty() &&
                ...
                Toast.makeText(applicationContext,
                      "Permiso denegado, no puede  utilizar esta funcion",
                Toast.LENGTH_LONG).show()
            }
        }
    ...
    private fun crearArchivoTemporal() : File{
        ...
        val directorioArchivo: File =   this.getExternalFilesDir(
    ...
    private fun obtenerUri(archivo: File): Uri{
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
            FileProvider.getUriForFile(applicationContext,
                "pe.idat.edu.apppermisocamara.fileprovider", archivo)
        }else{
